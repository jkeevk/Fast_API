...
@app.get("/api/v1/advertisement",response_model=list[GetAdvertisementResponse],tags=["advertisements"])
async def get_advertisement_by_qs(session: SessionDependency,
    ...
    advertisement_list = [advert.dict for advert in advertisement_obj.unique().scalars().all()]
...
@app.get("/api/v1/user/{user_id}", 
         response_model=GetUserResponse, tags=["users"])
